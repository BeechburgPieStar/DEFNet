...
class ImageLidarDataset(data.Dataset):
    ...
    def _read_data(self, image_id):
        ...
        return img, mask, lidar, image_id  # 返回 image_id
    ...
    def __getitem__(self, index):
        ...
        img, mask, lidar, image_id = self._read_data(image_id)  # 获取 image_id
        img, mask = self._data_augmentation(img, mask, lidar)
        img, mask = torch.Tensor(img), torch.Tensor(mask)
        return img, mask, image_id  # 返回 image_id
...
class ImageGPSDataset(data.Dataset):
    def __init__(self, image_list, sat_root, mask_root, gps_root, sat_suffix="png", mask_suffix="png", pgs_suffix="jpg",
                 randomize=True, down_scale=True, down_resolution=512):
        self.image_list = image_list
    ...
    def _read_data(self, image_id):
        ...
        return img, mask, gps, image_id  # 返回 image_id
    ...
    def __getitem__(self, index):
        image_id = self.image_list[index]
        img, mask, gps, image_id = self._read_data(image_id)  # 获取 image_id
